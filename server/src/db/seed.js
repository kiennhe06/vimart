...
async function seed() {
  ...
  const admin = await insertUser('admin@vimart.vn', password, 'Quản Trị Viên', '0900000001', 'admin');
  const seller1 = await insertUser('seller1@vimart.vn', password, 'Trần Văn Bán', '0900000002', 'customer');
  const seller2 = await insertUser('seller2@vimart.vn', password, 'Lê Thị Shop', '0900000003', 'customer');
  const buyer = await insertUser('buyer@vimart.vn', password, 'Nguyễn Văn Mua', '0900000004', 'customer');
  ...
  const shop1 = await insertShop(seller1, 'Shop Táo Xanh', 'Chuyên phụ kiện & điện thoại chính hãng', null);
  const shop2 = await insertShop(seller2, 'Thời Trang GenZ', 'Quần áo phong cách trẻ trung, giá tốt', null);

  console.log('⏳ Nạp sản phẩm...');
  const img = (seed) => `https://picsum.photos/seed/${seed}/600/600`;

  await insertProduct(shop1, cats['dien-thoai'], 'Ốp lưng iPhone 15 chống sốc', 'Ốp silicon dẻo, chống bám vân tay', img('case15'), [
    { name: 'Đen', price: 89000, stock: 120 },
    { name: 'Trong suốt', price: 89000, stock: 80 },
    { name: 'Xanh', price: 99000, stock: 40 },
  ]);
  await insertProduct(shop1, cats['dien-thoai'], 'Sạc nhanh USB-C 20W', 'Củ sạc nhanh chuẩn PD, an toàn', img('charger'), [
    { name: 'Trắng', price: 149000, stock: 200 },
  ]);
  await insertProduct(shop1, cats['dien-thoai'], 'Tai nghe Bluetooth TWS Pro', 'Chống ồn, pin 6 giờ, kết nối nhanh', img('earbuds'), [
    { name: 'Trắng', price: 399000, stock: 60 },
    { name: 'Đen', price: 399000, stock: 55 },
  ]);
  await insertProduct(shop1, cats['gia-dung'], 'Đèn LED để bàn cảm ứng', 'Ba mức sáng, cổng USB tiện lợi', img('lamp'), [
    { name: 'Mặc định', price: 259000, stock: 30 },
  ]);

  await insertProduct(shop2, cats['thoi-trang'], 'Áo thun cotton unisex', 'Vải cotton 100%, form rộng thoải mái', img('tshirt'), [
  ...
  await insertProduct(shop2, cats['thoi-trang'], 'Quần jeans nam basic', 'Co giãn nhẹ, dễ phối đồ', img('jeans'), [
    { name: 'Size 29', price: 289000, stock: 25 },
    { name: 'Size 30', price: 289000, stock: 30 },
    { name: 'Size 31', price: 289000, stock: 20 },
  ]);
  await insertProduct(shop2, cats['lam-dep'], 'Son kem lì lâu trôi', 'Chất son mịn, lên màu chuẩn', img('lipstick'), [
    { name: 'Đỏ cam', price: 159000, stock: 45 },
    { name: 'Hồng đất', price: 159000, stock: 40 },
  ]);
  await insertProduct(shop2, cats['sach'], 'Sách "Tư duy nhanh và chậm"', 'Bản dịch tiếng Việt, bìa mềm', img('book'), [
    { name: 'Mặc định', price: 119000, stock: 15 },
  ]);

  console.log('✅ Nạp dữ liệu xong.');
  console.log('   Tài khoản test (mật khẩu 123456): admin@vimart.vn, seller1@vimart.vn, seller2@vimart.vn, buyer@vimart.vn');
  await pool.end();
}
